...
function Drop({ file, mask, accent, onFile }: { file: File | null; mask?: boolean; accent: string; onFile: (file: File) => void }) {
  ...
  return <button type="button" onClick={() => input.current?.click()} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const next = event.dataTransfer.files[0]; if (next) onFile(next); }} className="group relative flex min-h-52 overflow-hidden rounded-2xl border border-dashed border-white/20 bg-black/25">
...
export function VideoControlWorkspace({ tool }: { tool: VideoControlTool }) {
  ...
  return <div className="grid overflow-hidden rounded-3xl border border-white/15 bg-[#151824] lg:grid-cols-[minmax(0,.94fr)_minmax(0,1.06fr)]">
    <section className="border-b border-white/15 bg-white/[.025] p-5 sm:p-7 lg:border-b-0 lg:border-r">
      <div className={tool.type === 'inpaint' ? 'grid gap-3 sm:grid-cols-2' : ''}><Drop file={video} accent={tool.accent} onFile={setVideo} />{tool.type === 'inpaint' && <Drop file={mask} mask accent={tool.accent} onFile={setMask} />}</div>
      <label className="mt-6 block text-sm font-semibold text-white/75">Describe the replacement</label><textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} rows={5} className="mt-2 w-full resize-none rounded-2xl border border-white/15 bg-[#171b27] p-4 text-sm leading-6 outline-none focus:border-white/35" />
      ...
      {tool.type !== 'inpaint' && <label className="mt-4 flex items-start gap-3 text-sm leading-5 text-white/60"><input type="checkbox" checked={preprocess} onChange={(event) => setPreprocess(event.target.checked)} className="mt-1" style={{ accentColor: tool.accent }} /><span>Extract {tool.type.toUpperCase()} from my normal video<small className="mt-1 block text-white/35">Turn this off only when the upload is already a prepared control pass.</small></span></label>}
      <label className="mt-4 flex items-start gap-3 rounded-xl border border-amber-200/15 bg-amber-200/[.04] p-3 text-xs leading-5 text-white/55"><input type="checkbox" checked={accepted} onChange={(event) => setAccepted(event.target.checked)} className="mt-1" /><span>I accept the <a className="underline" target="_blank" rel="noreferrer" href="https://huggingface.co/alibaba-pai/MiniMax-H3-Fun-Controlnet-Union/blob/main/LICENSE">MiniMax H3 Community License</a> and its use restrictions. This model is unavailable in the US, EU, UK, and South Korea.</span></label>
      <div className="mt-5 flex items-center justify-between rounded-xl border border-white/10 bg-white/[.025] px-4 py-3"><span><b className="block text-sm">Estimated price</b><small className="text-white/40">Final price is 1.2× measured compute</small></span><strong className="text-right text-lg">~{Math.ceil(estimate * 100)} credits<small className="block text-xs font-normal text-white/35">${estimate.toFixed(2)}</small></strong></div>
      <button onClick={() => void generate()} disabled={busy || territoryAllowed !== true} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl px-5 py-3.5 text-sm font-bold text-black disabled:opacity-55" style={{ background: tool.accent }}>{busy ? <Loader2 className="animate-spin" size={17} /> : <WandSparkles size={17} />}{busy ? status || 'Generating…' : territoryAllowed === false ? 'Unavailable in this territory' : territoryAllowed === null ? 'Checking availability…' : `Generate with ${tool.type.toUpperCase()} control`}</button>
