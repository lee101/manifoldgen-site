    ...
    <main className="min-h-screen bg-[var(--color-ink)] text-white">
      ...
      <div className="mx-auto max-w-7xl px-5 py-10 md:py-14">
        ...
        <section className="grid gap-8 py-10 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-end">
          <div><div className="text-xs font-semibold uppercase tracking-[0.2em]" style={{ color: control.accent }}>MINIMAX H3 CONTROLNET UNION · {control.eyebrow}</div><h1 className="mt-4 max-w-4xl font-display text-4xl font-700 tracking-tight md:text-6xl">{control.name}</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-white/70">{control.description}</p></div>
          <div className="rounded-2xl border border-white/15 bg-white/[0.05] p-5"><div className="text-xs uppercase tracking-[0.16em] text-white/50">Control behavior</div><div className="mt-3 space-y-2">{control.bestFor.map((strength) => <div key={strength} className="flex items-center gap-2 text-sm text-white/75"><Check size={14} style={{ color: control.accent }} /> {strength}</div>)}</div><div className="mt-4 border-t border-white/15 pt-4 text-sm text-white/60">1–15s · fixed 24 fps · final price follows measured GPU compute</div></div>
        </section>
