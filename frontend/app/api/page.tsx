...
Optionally redraw the first frame first with \`{"service":"image-edit","image_url":"https://...png","prompt":"recreate this frame with the characters swapped ...","width":1536,"height":1024,"n":1}\`, then POST \`/api/service\` with \`{"service":"character_swap_video","video_url":"https://...mp4","image_url":"https://...png","prompt":"Image 1 is the exact target look: Elon Musk on the left ... Video 1 is only the motion reference; from the very first frame show only the characters of Image 1 ...","resolution":"768P"}\`. Name the new characters and the background explicitly in \`prompt\`; H3 follows the motion of Video 1 far more reliably when the prompt says who replaces whom. The 5–60 s source is split into ≤4.5 s segments rendered in parallel as 5 s H3 reference clips (the copied opening frames are trimmed from each head), every clip is checked by a vision model and regenerated up to twice if the original performers leak through (\`result.clip_retries\`; retries are not billed), and the output is a 1280x720 24 fps H.264/AAC MP4 with the untouched original soundtrack. Pricing is $0.096 per generated second at 768P, $0.156 at 2K, plus $0.24 when \`image_url\` is omitted and \`character_prompt\` is used. Preflight with \`POST /api/character-swap/estimate\` (\`video_url\`, \`image_url\`, \`resolution\`, \`prompt\`), then poll \`GET /api/video-jobs/{job_id}\` for \`result.video_url\`, \`result.swapped_image_url\`, and \`result.chunks_completed\`.
