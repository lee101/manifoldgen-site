...
import { ArrowLeft, ArrowRight, BookOpen, Clock3, KeyRound } from 'lucide-react';
    ...
    <main className="min-h-screen bg-[var(--color-ink)] text-white">
      ...
      <div className="mx-auto max-w-7xl px-5 py-10 md:py-14">
        ...
        <div className="grid gap-10 py-10 lg:grid-cols-[minmax(0,1fr)_300px]">
          ...
          <div className="space-y-3"><Fact icon={<KeyRound size={16} />} label="Authentication" value="Bearer API key" /><Fact icon={<Clock3 size={16} />} label="Response" value="Async job + status URL" />{generator.guide && <Link href={`/blog/${generator.guide.slug}`} className="flex items-center gap-3 rounded-2xl border border-white/15 bg-white/[0.05] p-4 text-sm font-semibold hover:border-white/40"><BookOpen size={16} className="text-[var(--color-accent-2)]" /> {generator.guide.title} <ArrowRight size={15} className="ml-auto" /></Link>}</div>
        </div>
