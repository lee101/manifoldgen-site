...
test('tools index lists the song generator and voice studio cards', async ({ page }) => {
  ...
  await expect(gridCard(page, /Manifold Music Generator/)).toHaveAttribute('href', '/tools/music-generator');
  ...
  await expect(gridCard(page, /Manifold Music Generator/)).toBeHidden();

  await page.getByTestId('tools-search').fill('music');
  await expect(gridCard(page, /Manifold Music Generator/)).toBeVisible();
});
