...
var allowedVideoModels = map[string]bool{
	...
	"minimax/h3-max/text-to-video":                  true,
	"minimax/h3-max/image-to-video":                 true,
	"ra2v":                                          true,
}
...
func h3JobService(req ServiceUsageRequest) string {
	if req.Service == "sfx_generation" || req.Size == "audio" {
		return "sfx_generation"
	}
	return "h3_video"
}

func h3PublicRequestService(req ServiceUsageRequest) string {
...
func handleH3VideoService(ctx *fasthttp.RequestCtx, req ServiceUsageRequest, user *User) {
	...
	if route.RunpodEndpointID != "" {
		...
		handleRunpodH3VideoService(ctx, req, user, route)
		return
	}
	if route.CogURL != "" {
		logH3Route(req.Prompt, route)
...
func h3RunpodAPIKey() string {
	return strings.TrimSpace(getEnv("H3_RUNPOD_API_KEY", os.Getenv("RUNPOD_API_KEY")))
}

func h3RunpodBaseURL() string {
...
func callH3Runpod(endpointID, suffix string, method string, input interface{}, output interface{}) (int, error) {
	key := h3RunpodAPIKey()
	if key == "" {
	...
	req, err := http.NewRequest(method, h3RunpodBaseURL()+"/"+url.PathEscape(endpointID)+suffix, body)
	if err != nil {
		return 0, err
	}
	req.Header.Set("Authorization", "Bearer "+key)
	req.Header.Set("Content-Type", "application/json")
	response, err := appNZVideoClient.Do(req)
	if err != nil {
		return 0, err
	}
	defer response.Body.Close()
	data, err := io.ReadAll(io.LimitReader(response.Body, maxGeneratedVideoBytes+(4<<20)))
	if err != nil {
		return response.StatusCode, err
	}
	if response.StatusCode >= 300 {
		return response.StatusCode, fmt.Errorf("RunPod H3 returned %d: %s", response.StatusCode, tailOutput(data))
	}
	if output != nil && json.Unmarshal(data, output) != nil {
		return response.StatusCode, fmt.Errorf("RunPod H3 returned invalid JSON")
	}
...
func h3AudioDuration(job *VideoJob) int {
...
func h3AudioID(job *VideoJob) string {
	...
	return "audio_" + strings.TrimPrefix(job.ID, "video_")
}

func prepareH3AudioResult(job *VideoJob, result []byte) []byte {
	if !h3AudioJob(job) {
		return result
	}
	...
	audioURL := extractVideoURLFromResultJSON(string(result))
	if audioURL == "" {
		return result
	}
...
func indexCompletedH3Asset(job *VideoJob, result []byte) {
	if !h3AudioJob(job) {
...
func processH3VideoJob(job *VideoJob) {
	if strings.HasPrefix(job.ProviderJobID, "fal-h3-") {
		processFalH3MaxJob(job)
		return
	}
...
func parseRunpodH3ProviderJob(value string) (endpointID, jobID string, ok bool) {
	...
	if len(parts) != 3 || parts[0] != "runpod" || parts[1] == "" || parts[2] == "" {
		return "", "", false
	}
...
func uploadH3RunpodVideo(ctx context.Context, video []byte, userID, contentType string) (string, error) {
	...
	if resp.StatusCode >= 300 {
		...
		return "", fmt.Errorf("R2 H3 video upload returned %d: %s", resp.StatusCode, tailOutput(body))
	}
...
func processRunpodH3VideoJob(job *VideoJob) {
	...
	if len(job.Result) > 0 {
		...
		if json.Unmarshal(job.Result, &persisted) == nil {
			if v, ok := persisted["_h3_variant"].(string); ok && v != "" {
				variant = v
			}
		}
	}
...
func processLocalH3VideoJob(job *VideoJob) {
	...
	cogURL, _ := input["_h3_cog_url"].(string)
	...
	resultMap := map[string]interface{}{
		...
		"predict_seconds": predictSeconds, "model_variant": variant,
	}
	cutePrice := getCUTEPriceUSD()
	if cutePrice <= 0 || math.IsNaN(cutePrice) || math.IsInf(cutePrice, 0) {
		_ = dbConn.UpdateVideoJob(job.ID, "payment_required", nil, "CUTE price unavailable; retry status after pricing recovers")
...
func retryH3VideoJob(job *VideoJob) error {
	...
	if route.Variant == h3NormalVariant {
		...
		if h3FalCanHandle(req) {
			...
			if err != nil {
				return fmt.Errorf("could not queue fal H3 Max retry: %w", err)
			}
			payload := persistH3Request(req, h3FalPayload(req))
...
func publicVideoJob(job *VideoJob) *VideoJob {
	...
	if len(job.Result) > 0 {
		...
		if json.Unmarshal(job.Result, &result) == nil {
			...
			if h3AudioJob(job) {
				if _, ok := result["audio_url"].(string); !ok {
					if videoURL, ok := result["video_url"].(string); ok {
						result["audio_url"] = videoURL
					}
				}
				delete(result, "video_url")
				kind := audioJobKind(job)
...
func cancelVideoProvider(job *VideoJob) error {
	...
	case strings.HasPrefix(providerID, "fal-h3-"):
		_, requestID, ok := parseFalH3ProviderJob(providerID)
		if !ok {
			return nil
		}
		endpoint := strings.TrimRight(falQueueBaseURL, "/") + "/minimax/h3-max/requests/" + url.PathEscape(requestID) + "/cancel"
