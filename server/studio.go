...
func handleStudioGenerateMusic(ctx *fasthttp.RequestCtx) {
	...
	if music3EndpointID() != "" {
		handleMusic3Generation(ctx, user, prompt, input.Lyrics, duration, "music", input.ServiceTier, input.Seed)
		return
	}
...
func handleMusicGenerationAs(ctx *fasthttp.RequestCtx, user *User, prompt string, duration int, service string) {
	...
	if music3EndpointID() != "" {
		handleMusic3Generation(ctx, user, prompt, "", duration, service, "standard", 0)
		return
	}
